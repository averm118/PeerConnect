...
public class ViewForum {
    ...
    protected static Button button_CreatePost = UiFactory.action(
            ActionSpec.of("Create Post", "bi-plus", ControllerForum::performCreatePost));
    protected static Button button_Return = UiFactory.action(
            ActionSpec.of("Return", "bi-arrow-left", ControllerForum::performReturn, "pc-button-secondary"));
    protected static Button button_Quit = UiFactory.action(
            ActionSpec.of("Quit", "bi-x", ControllerForum::performQuit, "pc-button-secondary"));
    ...
    public static void displayForum(Stage ps, User user) {
        ...
        if (theView == null) {
            theView = new ViewForum();
        }

        populateThreadList();
        populatePostList("ALL");
        PeerConnectShell.show(theStage, theForumScene, "PeerConnect: Discussion Forum");
    }
    ...
    public ViewForum() {
        ...
        VBox threadPanel = UiFactory.card(
                UiFactory.section("Threads", listView_Threads));
        ...
        VBox postPanel = UiFactory.card(
                UiFactory.section("Posts", filterBar, listView_Posts));
        ...
        theForumScene = PeerConnectShell.scene(
                ScreenSpec.of("Discussion Forum", "Find threads, follow unread activity, and keep the conversation moving.",
                        theUser, "Forum", "bi-chat-square-text"),
                screen);
    }
    ...
    private static void openSelectedPost() {
        ...
        String currentThread = listView_Threads.getSelectionModel().getSelectedItem();
        populatePostList(currentThread);
        guiPost.ViewPost.displayPost(ViewForum.theStage, ViewForum.theUser, selectedPost);
    }
