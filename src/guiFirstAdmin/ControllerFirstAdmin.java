...
public class ControllerFirstAdmin {
	...
	*/
	...
	protected static void doSetupAdmin(Stage ps, int r) {
		//Check first admin username is valid
		//** Display Warning if not (tells which error it is).
		String userNameValid = UserNameRecognizer.checkForValidUserName(adminUsername);
		if(userNameValid.length() > 0) {
			ViewFirstAdmin.alertUsernamePasswordError.setTitle("Invalid Username\n.");
			ViewFirstAdmin.alertUsernamePasswordError.setHeaderText(userNameValid);
			ViewFirstAdmin.alertUsernamePasswordError.showAndWait();
			return;
		}
		
		//Display warning if password fail requirement
		String passWordValid = ModelChangePassword.evaluatePassword(adminPassword1);
		if(passWordValid.length() > 0) {
			ViewFirstAdmin.alertUsernamePasswordError.setTitle("Invalid Password\n.");
			ViewFirstAdmin.alertUsernamePasswordError.setHeaderText(passWordValid);
			ViewFirstAdmin.alertUsernamePasswordError.showAndWait();
			return;
		}
		...
		if (adminPassword1.compareTo(adminPassword2) == 0) {
            ...
            // User was established in the database, so navigate to log in
