...
public class ModelChangePassword {
	...
	protected static void updatePassword() {
		ViewChangePassword.resetAssessments();						// Reset the assessment flags to the
		String password = ViewChangePassword.text_Password.getText();	// initial state and fetch the input
		...
		if (password.isEmpty()) {
			ViewChangePassword.errPasswordPart1.setText("");
			ViewChangePassword.errPasswordPart2.setText("");
			ViewChangePassword.errPasswordPart3.setText("");
			ViewChangePassword.noInputFound.setText("No input text found!");
			
			ViewChangePassword.validPassword.setText("");
		}
		else
		{
			...
			if (errMessage != "") {
				...
				ViewChangePassword.noInputFound.setText("");			// There was input, so no error message
				
				// Extract the input up to the point of the error and place it in Part 1
				ViewChangePassword.errPasswordPart1.setText(password.substring(0, passwordIndexofError));
				...
				ViewChangePassword.validPassword.setTextFill(Color.RED);
				ViewChangePassword.validPassword.setText("Failure! The password is not valid.");
				
				// Ensure the button is disabled
				ViewChangePassword.button_Finish.setDisable(true);
			}
			else {
				...
				ViewChangePassword.validPassword.setTextFill(Color.GREEN);
				ViewChangePassword.validPassword.setText("Success! The password satisfies the requirements.");
				...
				ViewChangePassword.button_Finish.setDisable(false);
			} 
		}
	}
	...
	public static String goodPassword = "";
	...
	private static void updateFlags() {
		if (foundUpperCase) {
			ViewChangePassword.label_UpperCase.setText("At least one upper case letter - Satisfied");
			ViewChangePassword.label_UpperCase.setTextFill(Color.GREEN);
		}

		if (foundLowerCase) {
			ViewChangePassword.label_LowerCase.setText("At least one lower case letter - Satisfied");
			ViewChangePassword.label_LowerCase.setTextFill(Color.GREEN);
		}

		if (foundNumericDigit) {
			ViewChangePassword.label_NumericDigit.setText("At least one numeric digit - Satisfied");
			ViewChangePassword.label_NumericDigit.setTextFill(Color.GREEN);
		}

		if (foundSpecialChar) {
			ViewChangePassword.label_SpecialChar.setText("At least one special character - Satisfied");
			ViewChangePassword.label_SpecialChar.setTextFill(Color.GREEN);
		}

		if (foundLongEnough) {
			ViewChangePassword.label_LongEnough.setText("At least eight characters - Satisfied");
			ViewChangePassword.label_LongEnough.setTextFill(Color.GREEN);
		}
		
		if (foundShortEnough) {
			ViewChangePassword.label_ShortEnough.setText("Less than sixteen characters - Satisfied");
			ViewChangePassword.label_ShortEnough.setTextFill(Color.GREEN);
		}
	}
	...
	public static String evaluatePassword(String input) {
		...
		//add error for too long
		if(!foundShortEnough)
			errMessage += "Correct Length";
