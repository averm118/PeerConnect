	...
	public static String checkForValidUserName(String input) {
		...
		while (running) {
			...
			case 1: 
				...
				//**  Changed to add "-", "_", "." original code only check period
				...
				// The execution of this state is finished
				//** Changed to if the size is larger than 32, the loop must stop
				if (userNameSize > 32)
					running = false;
				break;			
		...
		case 0:
			...
			//** Changed to start must with A-Z, a-z
		...
		case 1:
			...
			//** swap orders, check if size > 32, then check for invalid character, then check if size < 4
			...
			if (userNameSize > 32) {
				...
				// There are characters remaining in the input, so the input is not valid. This check for valid characters.
				userNameRecognizerErrorMessage += 
		...
		case 2:
			...
			//** Changed from period to special characters
			userNameRecognizerErrorMessage +=
