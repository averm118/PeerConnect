...
public class ControllerNewAccount {
	...
	*/
	...
	protected static void doCreateUser() {
		...
		//** This set new variable to check if username is valid, return error message or nothing
		...
		if(userNameValid.length() > 0) {
			...
			ViewNewAccount.alertUsernamePasswordError.setHeaderText(userNameValid);
		...
		int roleCode = 0;
		User user = null;
		//check password valid and send error
		String passWordValid = ModelChangePassword.evaluatePassword(password);
		if(passWordValid.length() > 0) {
			ViewNewAccount.alertUsernamePasswordError.setTitle("Invalid Password\n.");
			ViewNewAccount.alertUsernamePasswordError.setHeaderText(passWordValid);
			ViewNewAccount.alertUsernamePasswordError.showAndWait();
			return;
		}
